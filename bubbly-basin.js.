alert(
    typeof pokemonDLC !== "undefined"
        ? `DLC geladen: ${pokemonDLC.length} Pokémon`
        : "FOUT: pokemonDLC is niet geladen"
);
...
function getPokemonDetailUrl(pokemon) {
    return `pokemon.html?section=dlc&number=${pokemon.number}`;
}


/* =========================================================
   ALLEEN BUBBLY BASIN DLC
   ========================================================= */

const bubblyBasinPokemon = pokemonDLC
...
function fillTypeFilter() {
    ...
    bubblyBasinPokemon.forEach((pokemon) => {
...
function createPokemonCard(pokemon) {
    ...
    return `
        <article class="pokemon-card">
            ...
            <p>
                <strong>🫧 Bubbly Basin DLC</strong>
            </p>
...
function renderPokemon() {
    ...
    const filtered = bubblyBasinPokemon.filter((pokemon) => {
        const number = String(pokemon.number).padStart(3, "0");
